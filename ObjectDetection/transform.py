...
def subplots(x, y1, y2, name, xlabel, ylabel):
    plt.plot(x, y1, label="transform with rotation")
    plt.plot(x, y2, label="transform with flit")
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(name)
    plt.legend()
...
def run(device, train_loader, val_loader, val_flip_loader, num_epochs, lr):
    ...
    for epoch in range(num_epochs):
        ...
        avg_val_loss_flip, avg_precision_flip, cprecisions_flip = val(model, device, val_flip_loader, criterion, 0.5)
        # val_accuracies.append(avg_precision)
        val_losses.append(avg_val_loss.item())
        cavg_p = sum(cprecisions)/len(cprecisions)
        class_wise_precisions.append(cavg_p)
        val_losses_flip.append(avg_val_loss_flip.item())
        class_wise_precisions_flip.append(sum(cprecisions_flip)/len(cprecisions_flip))
    ...
    subplots(range(1, num_epochs+1), val_losses, val_losses_flip, "val loss with different transform", "epoch", "loss")
    subplots(range(1, num_epochs+1), avg_val_precision, class_wise_precisions_flip, "val precision with different transform", "epoch", "precision")
...
# get best threshold
    ...
    with torch.no_grad():
        ...
        for t in t_s:
            pred = (output_f > t).astype(int)
            print(np.array_equal(pred, targets_f))
...
def main(num_epochs, batch_size_train, batch_size_val, learning_rate, device):
    ...
    transform_with_flip = transforms.Compose([transforms.RandomResizedCrop(224),
                             transforms.RandomVerticalFlip(0.5),
                             transforms.ToTensor(),
                             transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                    std=[0.229, 0.224, 0.225])])

    transform_with_rotate = transforms.Compose([transforms.RandomResizedCrop(224),
    ...
    val_set = ImgDataset('./VOCdevkit/VOC2012', transform_with_rotate, dataset='val')
    val_loader = DataLoader(val_set, batch_size=batch_size_val, shuffle=False)
    val_flip_set = ImgDataset('./VOCdevkit/VOC2012', transform_with_flip, dataset='val')
    val_flip_loader = DataLoader(val_flip_set, batch_size=batch_size_val, shuffle=False)

    criterion = nn.BCEWithLogitsLoss()

    run(device, train_loader, val_loader, val_flip_loader, num_epochs, learning_rate)
